use std::{borrow::Borrow, io::{Error, Result}};

use elasticsearch::{http::transport::Transport, indices::{IndicesCreateParts, IndicesExistsParts}, BulkIndexOperation, BulkOperation, BulkParts, Elasticsearch, IndexParts};
...
impl ElasticIndexMapping {
    pub fn new<I: AsRef<str>>(index: I, map: Value) -> Self {
        Self { name: index.as_ref().to_string(), map }
    }
...
impl ElasticsearchClient {
        ...
        match result {
            Ok(response) => {
                if !response.status_code().is_success() {
                    return Err(Error::new(std::io::ErrorKind::UnexpectedEof, response.status_code().as_str()))
                }
            },
            Err(err) => {
                return Err(Error::new(std::io::ErrorKind::UnexpectedEof, err.to_string()))
            },
        }
        Ok(Self {client})
    }
    ...
    async fn indice_exists(&self, indice: &str) -> bool {
        ...
        return match client.indices().exists(IndicesExistsParts::Index(&[indice])).send().await {
            Ok(result) => {
    ...
    pub async fn insert_many<R: Serialize, D: IntoIterator<Item = R>>(&self, index_name: &str, documents: D) {
        let client = self.client.clone();
        
        let operations = documents.into_iter().map(|item| {
